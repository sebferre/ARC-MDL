...
let solved_train_names = (* 12 tasks *)
  [ "ba97ae07.json"; (* two rectangles overlapping, below becomes above, runtime=21s *)
    "bda2d7a6.json"; (* nested squares, color shift, partial success: rare case seen as noise, pb: sensitive to params, not really understood, runtime=18s *)
    "5582e5ca.json"; (* 3x3 grid, keep only majority color, runtime=1s *)
    "e9afcf9a.json"; (* two one-color rows, interleaving them, runtime=0.5s *)
    "6f8cd79b.json"; (* black grid => add cyan border, runtime=0.2s *)
    "e48d4e1a.json"; (* colored cross moved according to height of grey rectangle at (0,9), runtime=142s *)
    "25ff71a9.json"; (* shape moving 1 pixel down, runtime=0.6s *)
    "1cf80156.json"; (* crop on shape, runtime=2.7s *)
    "aabf363d.json"; (* shape and point => same shape but with point color, runtime=2.6s, FAIL, pb: missing point, should accept loss on input for gain on output *)
    "b1948b0a.json"; (* any bitmap, changing background color, runtime=0.8s *)
    "bdad9b1f.json"; (* red and cyan segments, made full lines, yellow point at crossing, runtime=14.2s, FAIL *)
