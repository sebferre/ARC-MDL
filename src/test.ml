...
let solved_train_names = (* 48 tasks, 1.6s/task for timeout=30s, max_nb_parses=64, max_refs=20, max_exprs=10000 *)
  [ "017c7c7b.json"; (* NEW some blue shape in 6x3 grid, resize alike to grid size 9x3 and color in blue. runtime=0.3s *)
    "05f2a901.json"; (* NEW two objects, a red and a cyan, the red object moves onto the cyan object, runtime=0.3s *)
    "08ed6ac7.json"; (* 4 grey bars, colored in size order, runtime=2.0s *)
    "0962bcdd.json"; (* NEW two kinds of bi-color flowers, growing in size, runtime=4.3s *)
    ...
    "28bf18c6.json"; (* NEW a shape, tile it 1x2 and crop on this. runtime=0.2s *)
    ...
    "4938f0c2.json"; (* NEW red shape on the top left corner of a 2x2 green square, add 3 symmetric copies of the red shape around the green square. runtime=3.7s *)
    "496994bd.json"; (* NEW two colored horizontal stripes at top, add their flipHeight symmetries at bottom. runtime=0.3s *)
    "4c5c2cf0.json"; (* NEW object next to some central x-cross, add 3 symmetric copies of the shape around the x-cross. runtime=3.1s *)
    ...
    "5ad4f10b.json"; (* NEW a big shape, points, return the shape scaled-down to 3x3 with same color as points. runtime=5.3s *)
    "681b3aeb.json"; (* 2 shapes, paving a 3x3 grid, a bit lucky. runtime=0.5s *)
    "694f12f3.json"; (* NEW 2 yellow rectangles, fill the larger one in red, the smaller in blue, runtime=6.1 *)
    "6b9890af.json"; (* NEW red border, colored shape; crop on border and fit colored shape inside. runtime=1.0s *)
    ...
    "952a094c.json"; (* NEW border with points inside, points moving outsite border on opposite corner, runtime=2.2s *)
    "98cf29f8.json"; (* NEW (SUCCESS on 2nd trial) two rectangles with a rectangular bridge, the rectangle with same color as bridge moves onto the other rectangle, runtime=0.4s *)
    ...
    "b1948b0a.json"; (* any bitmap, changing background color, runtime=0.1s *)
    "b230c067.json"; (* NEW three cyan shapes, set the two biggest blue (same shapes), and the smallest red, runtime=1.5s *)
    ...
    "b9b7f026.json"; (* NEW several ractangles, only one not full, generate a 1-cell grid with same color as non-full rectangle, runtime=1.5s *)
    "ba97ae07.json"; (* two rectangles overlapping, below becomes above, runtime=0.2s *)
    "bb43febb.json"; (* NEW two grey rectangles, add red rectangles inside, letting a grey border, runtime=8.3s *)
    ...
    "d631b094.json"; (* NEW any colored shape, output 1 x area grid with same color, runtime=0.1s *)
    "dc433765.json"; (* NEW a gren point and a yellow point, the green one moves one step towards the green one, runtime=0.1s *)
    "e48d4e1a.json"; (* colored cross moved according to height of grey rectangle at (0,9), runtime=1.6s *)
    "e9614598.json"; (* NEW two aligned blue points, add a 3x3 green +-cross in between, runtime=0.3s *)
    ...
    "ed36ccf7.json"; (* NEW black object, apply rotate270 symmetry to mask and position relative to grid. runtime=0.3s *)
    "f25fbde4.json"; (* NEW yellow shape, double its size and crop on it. runtime=0.1s *)
  ]
