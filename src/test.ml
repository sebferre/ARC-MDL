...
let solved_train_names = (* 46 tasks, 5.4s/task for timeout=30s, max_nb_parses=64, max_refs=50, max_exprs=10000 *)
  [ "017c7c7b.json"; (* NEW some blue shape in 6x3 grid, resize alike to grid size 9x3 and color in blue. runtime=3.3s *)
    ...
    "1bfc4729.json"; (* 2 colored points, expand each in a fixed shape at relative position, runtime=2.0s *)
    ...
    "7e0986d6.json"; (* collection of rectangles + noise points to be removed, runtime=20.6 *)
    "952a094c.json"; (* NEW border with points inside, points moving outsite border on opposite corner, runtime=4.1 *)
    "98cf29f8.json"; (* NEW (SUCCESS on 2nd trial) two rectangles with a rectangular bridge, the rectangle with same color as bridge moves onto the other rectangle, runtime=0.6 *)
    "a1570a43.json"; (* red shape moved into 4 green points, runtime=5.7s *)
    "a61ba2ce.json"; (* 4 corners, they join as a small square, runtime=8.8s *)
    "a79310a0.json"; (* cyan shape, moving 1 pixel down, runtime=0.0s *)
    "a87f7484.json"; (* crop on the largest 3x3 shape. runtime=0.5s *)
    ...
    "b230c067.json"; (* NEW three cyan shapes, set the two biggest blue (same shapes), and the smallest red, runtime=12.9s *)
    ...
    "b9b7f026.json"; (* NEW several ractangles, only one not full, generate a 1-cell grid with same color as non-full rectangle, runtime=5.8s *)
    "ba97ae07.json"; (* two rectangles overlapping, below becomes above, runtime=1.1s *)
    "bb43febb.json"; (* NEW two grey rectangles, add red rectangles inside, letting a grey border, runtime=2.3s *)
    "bda2d7a6.json"; (* nested squares, color shift, partial success: rare case seen as noise, pb: sensitive to params, not really understood, runtime=4.2s. With collection: need access to color of item at position (i - 1) mod 3 *)
    ...
    "bdad9b1f.json"; (* red and cyan segments, made full lines, yellow point at crossing, runtime=1.7s *)
    "be94b721.json"; (* 3 shapes (at least), selecting the biggest one, runtime=1.5s *)
    ...
    "e48d4e1a.json"; (* colored cross moved according to height of grey rectangle at (0,9), runtime=2.3s *)
    "e9614598.json"; (* NEW two aligned blue points, add a 3x3 green +-cross in between, runtime=0.6s *)
