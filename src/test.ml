...
let solved_train_names = (* 28 tasks, 27/26 success, 256s for timeout=60s, max_nb_parses=64 *)
  [ "08ed6ac7.json"; (* 4 grey bars, colored in size order, runtime=27.5s *)
    ...
    "1f85a75f.json"; (* crop of a shape among a random cloud of points. runtime about 3s, timeout trying to explain everything *)
    ...
    "445eab21.json"; (* output a 2x2 grid with color from the larger rectangle. runtime=3.3s *)
    "48d8fb45.json"; (* crop on one shape among several, should choose next to grey point but works by choosing 2nd layer (decreasing size). runtime=8.5s *)
    "5521c0d9.json"; (* three rectangles moving up by their height, runtime=6.9s. *)
    "5582e5ca.json"; (* 3x3 grid, keep only majority color, runtime=5.0s *)
    "681b3aeb.json"; (* 2 shapes, paving a 3x3 grid, a bit lucky. runtime=4.1s *)
    "6f8cd79b.json"; (* black grid => add cyan border, runtime=0.2s *)
    "7e0986d6.json"; (* NEW collection of rectangles + noise points to be removed *)
    "91714a58.json"; (* FAIL find the main rectangle in a cloud of points, runtime=43.5. pb: succeeds on training examples but fails on test, not clear why exactly *)
    "a1570a43.json"; (* red shape moved into 4 green points, runtime=26.9s *)
    "a61ba2ce.json"; (* NEW 4 corners, they join as a small square, runtime=17s *)
    "a79310a0.json"; (* cyan shape, moving 1 pixel down, runtime=0.3s *)
    "a87f7484.json"; (* crop on the largest 3x3 shape. runtime=16.6s *)
    ...
    "b94a9452.json"; (* square in square, crop on big square, swap colors, runtime=1.2s *)
    "ba97ae07.json"; (* two rectangles overlapping, below becomes above, runtime=5.1s *)
    "bda2d7a6.json"; (* nested squares, color shift, partial success: rare case seen as noise, pb: sensitive to params, not really understood, runtime=5.7s. With collection: need access to color of item at position (i - 1) mod 3 *)
...
let maybe_train_names =
  [
    ...
    "d5d6de2d.json"; (* TODO pb: collection, fails to generalize from mapping first item to all items (use relax mechanism) *)
