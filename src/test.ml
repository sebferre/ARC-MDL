...
let solved_train_names = (* 12 tasks *)
  [ "ba97ae07.json"; (* two rectangles overlapping, below becomes above, runtime=28s *)
    "bda2d7a6.json"; (* nested squares, color shift, partial success: rare case seen as noise, pb: sensitive to params, not really understood, runtime=37s *)
    "5582e5ca.json"; (* 3x3 grid, keep only majority color, runtime=6s *)
    "e9afcf9a.json"; (* two one-color rows, interleaving them, runtime=1.1s *)
    "6f8cd79b.json"; (* black grid => add cyan border, runtime=0.4s *)
    "e48d4e1a.json"; (* colored cross moved according to height of grey rectangle at (0,9), runtime=timeout FAIL *)
    "25ff71a9.json"; (* shape moving 1 pixel down, runtime=1.3s *)
    "1cf80156.json"; (* crop on shape, runtime=3.9s *)
    "aabf363d.json"; (* shape and point => same shape but with point color, runtime=4.2s, FAIL, pb: missing point, should accept loss on input for gain on output *)
    "b1948b0a.json"; (* any bitmap, changing background color, runtime=1.5s *)
    "bdad9b1f.json"; (* red and cyan segments, made full lines, yellow point at crossing, runtime=57s, FAIL *)
    "a79310a0.json"; (* cyan shape, moving 1 pixel down, 1.8s *)
  ]
