...
\begin{table}[t]
  \centering
  \caption{Performance on training and evaluation tasks as the number of tasks solved, per version ($\alpha = 10$ until v3.2 then $100$)}
...
%    v2.2 & 2021-10-11 & 30s & 15s & ~44 / 52.5~ & ~30 / 30.5~ & 21s & ~9 / 13.2 & ~7 / 7.0~ \\
    ...
    \hline
    v2.7 & 2022-12-20 & 60s & 22s & ~102 / 112.6~ & ~96 / 96.5~ & 35s & ~32 / 39.9~ & ~23 / 23.5~ \\
    \hline
    \hline
    v3.0 & 2023-12-10 & 60s & 55s & ~85 / 88.3~ & ~77 / 77.5~ & 45s & ~31 / 35.5~ & ~27 / 18.0~ \\
    \hline
    v3.1 & 2024-02-26 & 60s & 32s & ~101 / 103.0~ & ~89 / 89.0~ & - & ~36 / -~ & ~32 / -~ \\
         &            & 120s & - & - & - & 62s & ~38 / 52.7~ & ~35 / 41.0~ \\
    \hline
    v3.2 & 2024-03-26 & 60s & 36s & ~111 / 114.3~ & ~99 / 100.0~ & 44s & ~47 / 53.5~ & ~41 / 42.0~ \\
    \hline
    v3.3 & 2024-05-05 & 60s & 42s & ~125 / 127.7~ & ~112 / 113.3~ & 48s & ~63 / 68.0~ & ~56 / 57.0~ \\
    \hline
    v3.4 & 2024-09-04 & 60s & 44s (7s) & ~134 / 141.4~ & ~117 / 118.8~ & 54s (15s) & ~64 / 74.3~ & ~53 / 55.0~ \\
    \hline
%    - & 2024-09-22 & 60s & 45s (6.3s) & ~139 / 146.0~ & ~122 / 123.8~ & 55s & ~63 / 73.0~ & ~55 / 56.5~ \\
%    - & 2024-10-18 & 120s & 86s (8.9s) & ~139 / 147.5~ & ~122 / 123.8~ & - & - & - \\
%    - & 2024-10-28 & 120s & 79s (12.2s) & ~140 / 148.2~ & ~126 / 127.3~ & - & - & - \\
    v3.5 & 2024-10-29 & 180s & 111s (18s) & ~146 / 154.4~ & ~127 / 128.2~ & 143s (37s) & ~69 / 79.6~ & ~59 / 60.5~ \\
    \hline
    v3.6 & 2025-03-09 & 120s & 79s (15s) & ~133 / 138.8~ & ~124 / 126.2~ & 96s (14s) & ~64 / 69.4~ & ~56 / 57.5~ \\
    \hline
     & 2025-04-03 & 120s & 70s (10s) & ~132 / 139.0~ & ~122 / 124.2~ & 82s (11s) & ~65 / 70.0~ & ~57 / 58.5~ \\
     & 2025-04-06 & 120s & 70s (14s) & ~147 / 151.5~ & ~127 / 129.2~ & 87s (10s) & ~71 / 76.5~ & ~63 / 64.5~ \\
     & 2025-04-08 & 120s & 85s (15s) & ~147 / 154.0~ & ~135 / 137.2~ & 103s (11s) & ~75 / 81.8~ & ~67 / 69.0~ \\
%     & 2025-05-02 & 120s & 84s (12s) & ~153 / 158.7~ & ~135 / 136.8~ & - & - & - \\
%     & 2025-05-27 & 120s & 84s (18s) & ~158 / 162.9~ & ~136 / 138.3~ & - & - & - \\
    v3.7 & 2025-06-03 & 120s & 86s (12s) & ~157 / 164.7~ & ~139 / 140.3~ & 109s (18s) & ~78 / 84.7~ & ~69 / 70.5~ \\
    \hline
  \end{tabular}
  \label{tab:eval}
\end{table}
...
\paragraph{Version 2.6 -- 2022-11-07.} Here are the main changes in this version:
\begin{itemize}
...
\paragraph{Version 2.7 -- 2022-12-20.}
\begin{itemize}
\item a lot of refactoring
\item patterns: grid tiling
\end{itemize}

\paragraph{Version 3.0 -- 2023-12-10.} Restart with the MADIL framework. New models:
lost incremental object addition and object overlapping; gained
sequences (of objects and other things); lifted a number of functions
as patterns for better incrementality and findability (motifs). Kind
of DFS search, considering up to top-3 refinements at the same path at
each step.

\paragraph{Version 3.1 -- 2024-02-26.}
\begin{itemize}
\item introduced loop vars
\item patterns: segmentation mode as argument for objects + derived field for all objects as one; small grids as color sequences or matrices (sequence of sequences); derived pure grid in motifs; maps (replace, swap, fixed-domain); crop of a grid expression; recoloring of a grid expression; new motifs (border, crosses, star, diamond)
\item functions: unrepeat to compress grids; tail and reverse of a sequence
\end{itemize}

\paragraph{Version 3.2 -- 2024-03-06.}
\begin{itemize}
\item patterns: line objects, defined by length and direction; more segmentation modes for objects (connect by 8, 4, 2 and/or color)
\item functions: abs and direction (sign) of ints/vecs; transpose of vecs; top/bottom/left/right halves of grids; mask of a grid (ignoring colors); minority color of a grid; self-compose takes a bgcolor; cardinal of a sequence
\item bounding interleaving of object lists for parsing
\item explicit conversions between different kinds of integers (expressions) to penalize thorough DL
\end{itemize}

\paragraph{Version 3.3 -- 2024-05-05.}
\begin{itemize}
\item Ndtrees: transpose, flatten, and rotate functions on first dimension
\item sequence lengths encoded by patterns as extra arguments, parsing and generation conform to them
\item improved handling of sequences when parsing values
\item generation revised to align with parsing
\item enumeration of expressions by increasing size, with size limited to 9
\item alpha increased from 10 to 100
\item patterns: Monocolor includes pattern FullMask for full grids; Metagrid for grids of grids and one-color frontiers; partial motifs (adding a mask); new motif Corners for four-corner-dots objects
\item functions: selfCompose can use color expression
\end{itemize}

\paragraph{Version 3.4 -- 2024-09-04.}
\begin{itemize}
\item MADIL: handling of sequences delegated to the domain (no more loops), simpler pattern semantics (pure data decompositions). This allows for custom sequence matching and computation. Utilities for element-wise computations on sequences of sequences, etc.
\item notion of Decompositions: patterns that do not compress but split into parts. They must be further refined until made compressive. Subset of patterns: Vec, Obj.
\item patterns: Motif split into two patterns, Multicolor and Bicolor;
\item patterns on sequences: Cons (decomposition), Repeat, Index, Range
\item functions on sequences: min, max, argmin, argmax
\item MCTS search through model refinement tree (search space)
\end{itemize}

\paragraph{Version 3.5 -- 2024-10-29.}
\begin{itemize}
\item expression-only arguments (patterns Crop, Recoloring, SeqIndex) are separated from part arguments; refinements apart from patterns and expressions, a mix between;
\item ASD: no more default constructor; binary functions can be declared as unary if only used with a constant argument;
\item patterns: ColorSeq/ColorMat made decompositions; colors as grid decomposition; Square sizes and moves; added noise argument to Objects; new motif Rings;  
\item functions: MiddleCenter; Top...Middle defined on grids; GridOfColorSeq and GridOfColorMat; aggregations Sum, MostCommon and LeastCommon sequence item; RelativePos and TranslationOnto for inter-object relations; Halves replaced XHalf; generic Cast function; 
\item new version of expression index: more systematic and avoiding binary expressions, relying on inter-object functions, constant arguments, and aggregations.
\end{itemize}

\paragraph{Version 3.6 -- 2025-03-09.}
\begin{itemize}
\item one expression index per variable because only unary functions, enables to cache;
\item introduction of type for value distributions, used for parsing and generation;
\item patterns: sorting argument in Objects; Affine (ax+b); 
\item functions: Quadrants; Strip; Colors of a grid/object; Border, Interior and Neighbors on objects;
\item lots of code cleaning and simplification;
\item tuned some parameters after experiments.
\end{itemize}

Last changes:
\begin{itemize}
\item 2025-04-03: ranking refinements based on estimate DL, bug fix about normlized DL; more affine expressions
\item 2025-04-06: no more decompositions, non-compressive steps with stall detection to avoid divergence (using DL EMA)
\item 2025-04-08: grouping expressions by size in index, and bounding size to maxexprsize in index instead of on lookup
\item 2025-05-02: pattern Object (single object + noise), not inserting Monocolor immediately with SameColor mode
\item 2025-05-05: connectedness taking into account for object's DL
\item 2025-05-06: constr Skyline (motif pattern)
\item 2025-05-07: aggregates Count, DistinctCount; constr Unique and functions UniqueVals and UniqueRanks
\item 2025-05-20: new Param constructor for fixed values across sequence items (motifs, segmentations, object orders)
\item 2025-06-03: exclusion of large motif moise (at most 1/3)
\end{itemize}
